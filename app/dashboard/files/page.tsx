...
export default function FilesPage() {
  ...
  useEffect(() => {
    load();
    fetch("/api/projects/current").then((r) => r.json()).then((d) => setProjectId(d.project?._id || ""));
  }, []);
            ...
            {files.map((f) => (
              <div key={f._id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "10px 0", borderBottom: "1px solid var(--border)" }}>
                <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                  ...
                  <NeoBadge tone="dim">v{f.version}</NeoBadge>
                  {f.previousVersionId && <NeoBadge tone="dim">rev</NeoBadge>}
                </div>
